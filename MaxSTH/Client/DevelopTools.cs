       ...
       //       soundList.Add("EMP_Blast,DLC_HEISTS_BIOLAB_FINALE_SOUNDS");
        ...
        async void clientTest1()
        {
            foreach (string sound in soundList)
            {
                ...
                PlaySoundFrontend(500, splitSound[0], splitSound[1], false);
                await Delay(6000);
                StopSound(500);
            }
        }

        [EventHandler("clientTest2")]
        async static void clientTest2(string vehName)
        {
            if(await DoesVehicleNameExist(vehName))
            await World.CreateVehicle(new Model(Game.GenerateHash(vehName)), Game.PlayerPed.GetOffsetPosition(new Vector3(0, 7, 0)), Game.PlayerPed.Heading);
            else TriggerEvent("chat:addMessage", new{color=new[]{255,153,153},args=new[]{$"Vehicle name doesn't exist."}});
        }
